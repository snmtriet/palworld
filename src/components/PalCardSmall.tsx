...
const PalCardSmall = ({
  ...
  return (
    <div className="pal" onClick={() => onSelectPal(pal)}>
      <div className="container">
        <div className="elements">
          {elements.map((item) => (
            <div key={item.name} className="element">
              <Image src={item.image} alt={item.name} height={24} width={24} />
            </div>
          ))}
        </div>
        <div className="image">
          <Image loading="lazy" src={image} alt={name} height={60} width={60} />
        </div>
...
export default memo(PalCardSmall);
